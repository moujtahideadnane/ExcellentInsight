...
export function InferenceEngine() {
  ...
  const triggerScan = () => {
    ...
    const sequence = [
      () => setInferenceLogs(prev => [...prev, "[AI] Isolating spatial anomaly cluster..."]),
      () => setInferenceLogs(prev => [...prev, "[AI] Cross-referencing 4.2B historical records..."]),
      () => setInferenceLogs(prev => [...prev, "[AI] Calculating Pearson correlation coefficients..."]),
      () => setInferenceLogs(prev => [...prev, "[OK] Sub-population divergence found. (p < 0.001)"]),
      () => {
  ...
  return (
    ...
    >
      <div className="mb-16 max-w-2xl">
        <div className="text-[10px] font-mono text-[#888888] py-1 px-2 border border-[#333333] rounded-[4px] w-fit mb-4">ARTIFACT IV : INTELLIGENCE</div>
        <h2 className="text-[32px] md:text-[48px] font-bold tracking-tight mb-4">Autonomous Inference.</h2>
        <p className="text-[#888888] text-[16px] leading-relaxed">
          The platform does not just aggregate data; it understands it. Our statistical engine continuously scans your datasets to surface hidden correlations, anomalies, and actionable insights in plain English.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-[#333333] border border-[#333333] rounded-[6px] overflow-hidden relative">
        
        {/* Left: The Topographical Model */}
        <div className="bg-[#050505] p-6 lg:p-8 flex flex-col gap-4 relative min-h-[400px] lg:min-h-0">
          <div className="flex items-center justify-between text-[#888888] text-sm font-mono border-b border-[#222222] pb-4 z-10 relative">
            <span className="flex items-center gap-2"><Target className="h-4 w-4" /> Spatial Clustering</span>
            <div className="flex items-center gap-2 text-[10px]">
              <span className="w-2 h-2 rounded-full bg-[#EDEDED]" /> Normal
              <span className="w-2 h-2 rounded-full bg-[#F5A623] ml-2" /> Outlier
            </div>
          </div>
          ...
          <div className="flex-1 relative bg-[#000000] rounded-[4px] border border-[#222222] overflow-hidden flex items-center justify-center p-4">
            <div className="w-full h-full relative" style={{ maxWidth: '400px', maxHeight: '300px' }}>
              ...
              >
                 {!targetScanned && (
                   <span className="text-[10px] font-mono bg-black/80 px-2 py-1 select-none pointer-events-none text-[#EDEDED]">CLICK TO SCAN</span>
                 )}
                 {targetScanned && (
                   <div className="absolute -top-6 left-0 text-[10px] font-mono text-[#00E5FF] bg-black px-1">LOCK ACQUIRED</div>
                 )}
              </div>
        ...
        <div className="bg-[#0A0A0A] p-6 lg:p-8 flex flex-col gap-4">
          <div className="flex items-center text-[#888888] text-sm font-mono border-b border-[#222222] pb-4">
            <span className="flex items-center gap-2"><BrainCircuit className="h-4 w-4" /> Inference Engine</span>
          </div>
                ...
                {insightReady && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 text-[#EDEDED] font-semibold text-[14px] mb-3">
                      <Sparkles className="h-4 w-4 text-[#F5A623]" /> Critical Finding
                    </div>
                    <p className="text-[#888888] text-[13px] leading-relaxed">
                      The selected cohort exhibits a <span className="text-white font-medium">314% higher churn probability</span> compared to the baseline. 
                      This variance strongly correlates (0.89) with users who experienced a latency timeout during onboarding in the last 14 days.
                    </p>
                    
                    <div className="mt-4 pt-4 border-t border-[#222222] flex items-center justify-between">
                      <span className="text-[10px] font-mono text-[#444444]">CONFIDENCE: 98.4%</span>
