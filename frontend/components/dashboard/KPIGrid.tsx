...
interface KPIGridProps {
  ...
  onDeleteKPI?: (index: number) => Promise<void>
}

const priorityOrder: Record<string, number> = { high: 0, medium: 1, low: 2 }

export default function KPIGrid({ kpis, onUpdateKPI, onDeleteKPI }: KPIGridProps) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
      {displayKpis.map((kpi) => (
        <InteractiveKPICard
          ...
          onDelete={onDeleteKPI ? () => onDeleteKPI(kpi.originalIndex) : undefined}
        />
      ))}
    </div>
  )
