...
const ZeroSchemaMatrix = dynamic(() => import('@/components/landing/artifacts/ingestion/ZeroSchemaMatrix').then(mod => ({ default: mod.ZeroSchemaMatrix })), {
  loading: () => <div className="min-h-[400px] flex items-center justify-center"><span className="text-ve-muted font-mono text-sm">Loading...</span></div>,
  ssr: false
})
...
function TerminalOutput() {
  ...
  return (
    <div className="w-full bg-ve-bg border border-ve-border rounded-md overflow-hidden font-mono text-[13px] leading-relaxed shadow-2xl relative">
      <div className="flex flex-col p-4 gap-1 min-h-[220px]">
        {lines.slice(0, visibleLines).map((line, i) => (
          <motion.div
            ...
            className={`flex gap-3 ${line.includes('[OK]') ? 'text-ve-text' : line.includes('[WARN]') ? 'text-ve-warning' : 'text-ve-muted'}`}
          >
            <span>›</span>
            <span>{line}</span>
          </motion.div>
        ...
        {visibleLines < lines.length && (
          <motion.div animate={{ opacity: [1, 0] }} transition={{ repeat: Infinity, duration: 0.8 }} className="w-2.5 h-4 bg-ve-btn-primary mt-1 ml-4" />
        )}
      </div>
      {/* Vercel Edge aesthetic: tiny technical labels */}
      <div className="absolute top-2 right-4 text-[10px] text-ve-muted">us-east-1</div>
    </div>
  )
}
...
export default function LandingClient() {
  ...
  return (
    <div className="min-h-screen bg-ve-bg text-ve-text selection:bg-ve-blue selection:text-white font-[family-name:var(--font-sans)] overflow-x-hidden">
      ...
      <nav className={cn(
        ...
        scrolled ? "bg-black/90 backdrop-blur-md border-b border-ve-border" : "bg-transparent"
      )}>
        ...
        <div className="hidden md:flex items-center gap-8 text-[13px] font-medium text-ve-muted">
        ...
        <div className="flex items-center gap-6 text-[13px] font-medium">
          {!isAuthenticated ? (
            <>
              <Link href="/login" className="text-ve-muted hover:text-white transition-colors">Login</Link>
              <MagneticButton>
                <Link href="/signup" className="bg-white text-black px-4 py-1.5 rounded-[6px] hover:bg-ve-btn-hover transition-colors flex items-center gap-2">
                  Deploy
                </Link>
              </MagneticButton>
            </>
          ) : (
            <Link href="/dashboard" className="bg-white text-black px-4 py-1.5 rounded-[6px] hover:bg-ve-btn-hover transition-colors flex items-center gap-2">
      ...
      <section className="relative min-h-[85vh] flex items-center py-24 lg:py-32 px-4 sm:px-6 lg:px-8 w-full max-w-7xl mx-auto scroll-tilt-target">
        <div className="flex-1 flex flex-col items-start gap-8 z-10 w-full mb-16 md:mb-0">
          ...
          <h1 className="text-[clamp(3.5rem,8vw,6rem)] font-bold leading-[0.9] tracking-tighter">
            ...
            <span className="text-ve-muted">into insights.</span>
          </h1>

          <p className="text-[16px] text-ve-muted max-w-md leading-relaxed">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="flex items-center gap-4 mt-4 w-full flex-wrap">
            <MagneticButton>
              <Link href="/signup" className="bg-white text-black px-6 py-3 rounded-[6px] text-sm font-medium hover:bg-ve-btn-hover transition-colors flex items-center gap-2">
                Analyze Your First File <ArrowRight className="h-4 w-4" />
              </Link>
            </MagneticButton>
            <Link href="/dashboard" className="px-6 py-3 rounded-[6px] text-sm font-medium text-ve-muted bg-ve-surface border border-ve-border hover:text-white hover:border-ve-muted transition-colors flex items-center gap-2">
        ...
        <div className="flex-1 w-full max-w-[600px] xl:max-w-none">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 w-full">
             {[
               ...
               <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 + i * 0.1 }} className="bg-ve-surface p-4 flex flex-col justify-center items-center gap-1 border border-ve-border rounded-[6px] hover:bg-ve-elevated hover:border-ve-muted transition-colors cursor-default">
                  <span className="font-mono text-[18px] font-medium text-white"><Counter target={s.val} suffix={s.suffix} /></span>
                  <span className="text-[10px] tracking-widest uppercase text-ve-muted">{s.l}</span>
               </motion.div>
             ))}
          </div>
        </div>
      ...
      {/* ── Call To Action ── */}
      <section className="py-24 lg:py-32 px-4 sm:px-6 lg:px-8 border-t border-ve-border-subtle w-full max-w-7xl mx-auto" id="get-started">
        <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-8" data-animate>
          ...
          <p className="text-ve-muted text-[16px] max-w-2xl leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mt-8 mb-8">
            <div className="flex flex-col items-center gap-2 p-4">
              <div className="text-ve-text font-mono text-[24px] font-bold">60s</div>
              <div className="text-ve-muted text-[13px]">Average analysis time</div>
            </div>
            <div className="flex flex-col items-center gap-2 p-4">
              <div className="text-ve-text font-mono text-[24px] font-bold">Auto</div>
              <div className="text-ve-muted text-[13px]">KPI detection powered by AI</div>
            </div>
            <div className="flex flex-col items-center gap-2 p-4">
              <div className="text-ve-text font-mono text-[24px] font-bold">Zero</div>
              <div className="text-ve-muted text-[13px]">Configuration required</div>
            </div>
          </div>

          <div className="flex items-center gap-4 mt-4">
            <MagneticButton>
              <Link href="/signup" className="bg-white text-black px-8 py-3 rounded-[6px] font-medium hover:bg-ve-btn-hover transition-colors text-sm">
                Start Free Analysis
              </Link>
            </MagneticButton>
            <Link href="/dashboard" className="px-8 py-3 rounded-[6px] font-medium text-white border border-ve-border hover:bg-ve-surface transition-colors text-sm">
              View Demo Dashboard
            </Link>
          </div>
        </div>
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-ve-border-subtle py-12 px-4 sm:px-6 lg:px-8 text-ve-muted text-[13px] flex flex-col md:flex-row items-center justify-between gap-6 max-w-7xl mx-auto w-full">
